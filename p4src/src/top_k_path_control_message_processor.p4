                        ...
                        inout standard_metadata_t standard_metadata)
{
    ...
    apply{
        if(hdr.packet_out.top_k_path_flags[7:7] == (bit<1>)1){ //this is a delete message
            ...
            log_msg("Values in the message are rank-{} port- {} rank_max_index -{} rank_minIndex- {}, new_port_index-{} bitmask -- {} torid {}",
                {hdr.packet_out.rank, hdr.packet_out.port, hdr.packet_out.rank_max_index, hdr.packet_out.rank_min_index, hdr.packet_out.new_port_index, hdr.packet_out.bitmask,hdr.packet_out.tor_id  });
            stored_bitmask.write( (bit<32>)hdr.packet_out.tor_id, hdr.packet_out.bitmask[K - 1 :0]);
            rank_to_max_index.write((bit<32>)(hdr.packet_out.rank+ (hdr.packet_out.tor_id * K)), hdr.packet_out.rank_max_index);
            rank_to_min_index.write((bit<32>)(hdr.packet_out.rank+ (hdr.packet_out.tor_id * K)), hdr.packet_out.rank_min_index);
