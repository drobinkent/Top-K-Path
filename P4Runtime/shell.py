    ...
    def find_table(self):
        for msg in reversed(self.stack):
            if msg.DESCRIPTOR.name == "TableEntry":
                try:
                    return self.dev.context.get_name_from_id(msg.table_id)
                except KeyError:
                    return None
        return None
    ...
    def find_action(self):
        for msg in reversed(self.stack):
            if msg.DESCRIPTOR.name == "Action":
                try:
                    return self.dev.context.get_name_from_id(msg.action_id)
                except KeyError:
                    return None
        return None


def _sub_object(dev, field, value):
    id_ = value
    try:
        return dev.context.get_name_from_id(id_)
    except KeyError:
        logger.info("Unknown object id {}".format(id_))


def _sub_mf(dev, field, value):
    id_ = field
    pcontext = _PrintContext(dev)
    table_name = pcontext.find_table()
    if table_name is None:
        logger.info("Cannot find any table in context")
        return
    return dev.context.get_mf_name(table_name, id_)


def _sub_ap(dev, field, value):
    id_ = field
    pcontext = _PrintContext(dev)
    action_name = pcontext.find_action()
    if action_name is None:
        logger.info("Cannot find any action in context")
        return
    return dev.context.get_param_name(action_name, id_)


def _gen_pretty_print_proto_field(dev, substitutions, pcontext):
    def myPrintField(self, field, value):
        ...
        if subs and field.name in subs and value != 0:
            name = subs[field.name](dev, field, value)
...
def _repr_pretty_proto(dev, msg, substitutions):
    ...
    google.protobuf.text_format._Printer.PrintField = _gen_pretty_print_proto_field(dev,
...
def _repr_pretty_p4info(dev, msg):
    substitutions = {
    ...
    return _repr_pretty_proto(dev,msg, substitutions)


def _repr_pretty_p4runtime(dev, id, msg):
    # substitutions = {
    #     "TableEntry": {"table_id": _sub_object(dev,id,msg)},
    #     "FieldMatch": {"field_id": _sub_mf(dev,id,msg)},
    #     "Action": {"action_id": _sub_object(dev,id,msg)},
    #     "Param": {"param_id": _sub_ap(dev,id,msg)},
    #     "ActionProfileMember": {"action_profile_id": _sub_object(dev,id,msg)},
    #     "ActionProfileGroup": {"action_profile_id": _sub_object(dev,id,msg)},
    #     "MeterEntry": {"meter_id": _sub_object(dev,id,msg)},
    #     "CounterEntry": {"counter_id": _sub_object(dev,id,msg)},
    #     "ValueSetEntry": {"value_set_id": _sub_object(dev,id,msg)},
    #     "RegisterEntry": {"register_id": _sub_object(dev,id,msg)},
    #     "DigestEntry": {"digest_id": _sub_object(dev,id,msg)},
    #     "DigestListAck": {"digest_id": _sub_object(dev,id,msg)},
    #     "DigestList": {"digest_id": _sub_object(dev,id,msg)},
    # }
    substitutions = {
        "TableEntry": {"table_id":  _sub_object},
...
class P4Object:
    def __init__(self, dev, obj_type, obj):
...
""".format(obj_type.pretty_name, self.name)
    ...
    def _repr_pretty_(self, p, cycle):
        p.text(_repr_pretty_p4info(self.p4dev, self._obj))

    def __str__(self):
        return _repr_pretty_p4info(self.p4dev, self._obj)
    ...
    def info(self):
        #print(_repr_pretty_p4info(self.p4dev, self._obj))
        return
    ...
    def actions(self):
        ...
        if self._obj_type == P4Type.table:
            for action in self._obj.action_refs:
                # print(self.p4dev.context.get_name_from_id(action.id))
                pass
        elif self._obj_type == P4Type.action_profile:
            t_id = self._obj.table_ids[0]
            t_name = self.p4dev.context.get_name_from_id(t_id)
            t = self.p4dev.context.get_table(t_name)
...
class P4Objects:
    def __init__(self,dev, obj_type):
        self._obj_type = obj_type
        self._names = sorted([name for name, _ in dev.context.get_objs(obj_type)])
        self._iter = None
        self.p4dev=dev
        self.__doc__ = """
...
""".format(pname=obj_type.pretty_name, pnames=obj_type.pretty_names, p4info=obj_type.p4info_name)
    ...
    def __getitem__(self, name):
        obj = self.p4dev.context.get_obj(self._obj_type, name)
        if obj is None:
            raise UserError("{} '{}' does not exist".format(
                self._obj_type.pretty_name, name))
        return P4Object(self.p4dev,self._obj_type, obj)
...
class Action:
    def __init__(self, dev, action_name=None):
        self._init = False
        self.p4dev = dev
        if action_name is None:
            raise UserError("Please provide name for action")
        self.action_name = action_name
        action_info = self.p4dev.context.get_action(action_name)
    ...
    def _from_msg(self, msg):
        ...
        self._params.clear()
        for p in msg.params:
            p_name = self.p4dev.context.get_param_name(self.action_name, p.param_id)
...
class _EntityBase:
    def __init__(self, dev, entity_type, p4runtime_cls, modify_only=False):
        self._init = False
    ...
    def __str__(self):
        self._update_msg()
        return str(_repr_pretty_p4runtime(self.p4dev, self.id,  self._entry))
#field, value, pcontext
    def _repr_pretty_(self, p, cycle):
        self._update_msg()
        p.text(_repr_pretty_p4runtime(self.p4dev, self.id, self._entry))
    ...
    def _write(self, type_):
        ...
        self.p4dev.client.write_update(update)
    ...
    def read(self, function=None):
        ...
        iterator = self.p4dev.client.read_one(entity)



        if function is None:
            return _EntryIterator(dev=self.p4dev,entity=self, it=iterator)
        else:
...
class _EntryIterator:
    def __init__(self, dev, entity, it):
        self._entity = entity
        self._it = it
    ...
    def __next__(self):
        ...
        if isinstance(self._entity, _P4EntityBase):
            e = type(self._entity)(self.p4dev,self._entity.name)  # create new instance of same entity
        else:
            e = type(self._entity)()
...
class _P4EntityBase(_EntityBase):
    def __init__(self,dev, p4_type, entity_type, p4runtime_cls, name=None, modify_only=False):
        super().__init__(dev, entity_type, p4runtime_cls, modify_only)
        self._p4_type = p4_type
        if name is None:
            raise UserError("Please provide name for {}".format(p4_type.pretty_name))
        self.name = name
        self._info = P4Objects(dev,p4_type)[name]
...
class ActionProfileMember(_P4EntityBase):
    def __init__(self, dev, action_profile_name=None):
        super().__init__(dev,
...
""".format(action_profile_name)
    ...
    def _from_msg(self, msg):
        ...
        if msg.HasField('action'):
            action = msg.action
            action_name = self.p4dev.context.get_name_from_id(action.action_id)
...
""".format(action_profile_name)
    ...
    def add(self, member_id=None, weight=1, watch=0):
        ...
        return self
    def del_member_from_group(self, member, weight=1, watch=0):
...
def _get_action_profile(dev, table_name):
    table = dev.context.get_table(table_name)
    implementation_id = table.implementation_id
    if implementation_id == 0:
        return None
    try:
        implementation_name = dev.context.get_name_from_id(implementation_id)
    except KeyError:
        raise InvalidP4InfoError(
            "Invalid implementation_id {} for table '{}'".format(
                implementation_id, table_name))
    ap = dev.context.get_obj(P4Type.action_profile, implementation_name)
    if ap is None:
...
class Oneshot:
    def __init__(self, dev, table_name=None):
        self._init = False
        self.p4dev = dev
        if table_name is None:
...
""".format(self.table_name)
    ...
    def _from_msg(self, msg):
        for action in msg.action_profile_actions:
            action_name = self.p4dev.context.get_name_from_id(action.action.action_id)
...
class TableEntry(_P4EntityBase):
    ...
    def __init__(self,dev, table_name=None):
        ...
        self.p4dev = dev
        ap = _get_action_profile(self.p4dev,table_name)
        if ap is None:
        ...
        for res_id in self._info.direct_resource_ids:
            ...
            if prefix == p4info_pb2.P4Ids.DIRECT_COUNTER:
                self._direct_counter = self.p4dev.context.get_obj_by_id(res_id)
            elif prefix == p4info_pb2.P4Ids.DIRECT_METER:
                self._direct_meter = self.p4dev.context.get_obj_by_id(res_id)
...
"""
    ...
    def _from_msg(self, msg):
        ...
        for mf in msg.match:
            mf_name = self.p4dev.context.get_mf_name(self.name, mf.field_id)
            self.match._mk[mf_name] = mf
        if msg.action.HasField('action'):
            action = msg.action.action
            action_name = self.p4dev.context.get_name_from_id(action.action_id)
            self.action = Action(self.p4dev,action_name)
            self.action._from_msg(action)
...
class CounterEntry(_CounterEntryBase):
    def __init__(self,dev, counter_name=None):
        super().__init__(dev,
...
class DirectCounterEntry(_CounterEntryBase):
    def __init__(self, dev, direct_counter_name=None):
        super().__init__(dev,
        ...
        self.p4dev = dev
        try:
            self._direct_table_name = self.p4dev.context.get_name_from_id(self._direct_table_id)
        except KeyError:
...
class MeterEntry(_MeterEntryBase):
    def __init__(self,dev, meter_name=None):
        super().__init__(dev,
...
class DirectMeterEntry(_MeterEntryBase):
    def __init__(self, dev, direct_meter_name=None):
        super().__init__(dev,
        ...
        try:
            ...
            raise InvalidP4InfoError("direct_table_id {} is not a valid table id".format(
                self._direct_table_id))
        self._table_entry = TableEntry(self.p4dev,self._direct_table_name)
        self.__doc__ = """
...
class P4RuntimeEntityBuilder:
    def __init__(self,dev, obj_type, entity_type, entity_cls):
        self.p4dev = dev
        self._obj_type = obj_type
        self._names = sorted([name for name, _ in self.p4dev.context.get_objs(obj_type)])
        self._entity_type = entity_type
...
class MulticastGroupEntry(_EntityBase):
    def __init__(self,dev, group_id=0):
        super().__init__(dev,
...
class CloneSessionEntry(_EntityBase):
    def __init__(self, dev, session_id=0):
        super().__init__(dev,
            P4RuntimeEntity.packet_replication_engine_entry,
            p4runtime_pb2.PacketReplicationEngineEntry)
